...
const CompleteProfile = ({
  ...
  const [userSpecialities, setUserSpecialities] = useState<
    OptionalInformation[]
  >([]);
  const [specialitiesForAdd, setSpecialitiesForAdd] =
    useState<OptionalInformation[]>(specialities);
  const [industriesForAdd, setIndustriesForAdd] =
    useState<OptionalInformation[]>(industries);
  ...
  const handleClearClick = (value: OptionalInformation) => {
    const arr = userIndustries.filter(
  ...
  const handleAutoComplete = (
    ...
    const value = industriesForAdd.find(
      (industrie: OptionalInformation) => industrie.name === newValue
    );
    if (value) {
  ...
  const {
    handleSubmit,
    handleChange,
    values: { name, companyName, phoneNumber },
    errors,
    getFieldProps,
    isSubmitting,
  } = useFormik({
  ...
  return (
    <Window>
      <form onSubmit={handleSubmit} className={classes.form}>
        ...
        <AddPosition onClick={handleClickOpen}>+ Add position</AddPosition>
        ...
        {industriesForAdd && (
          <Autocomplete
            ...
            disableClearable
            options={industriesForAdd.map(
              (industrie: OptionalInformation) => industrie.name
            )}
        ...
        <DialogPopup
          ...
          open={open}
          onClose={handleClose}
          specialities={specialitiesForAdd}
